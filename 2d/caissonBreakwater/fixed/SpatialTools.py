...
class ShapeRANS(Shape):
    ...
    def setFriction(self, friction, m_static, m_dynamic):
        """
        Sets constraints on the Shape (for moving bodies)

        :param m_static: static friction
        :param m_dynamic: dynamic friction
        """
    ...
    def setMass(self, mass):
        ...
        if self.volume:
            self.density = old_div(self.mass,self.volume)
    ...
    def setAbsorptionZones(self, flags, epsFact_solid, center, orientation,
                           dragAlpha=old_div(0.5,1.005e-6), dragBeta=0.,
                           porosity=1.):
        """
    ...
    def setPorousZones(self, flags, epsFact_solid, dragAlpha=old_div(0.5,1.005e-6),
...
class Tank3D(ShapeRANS):
    ...
    def setDimensions(self, dim):
        ...
        regions = [[old_div(((x0+frontSponge)+(x1-backSponge)),2.),
                    old_div(((y0+leftSponge)+(y1-rightSponge)),2.),
                    old_div((z0+z1),2.)]]
        regionFlags = [1]
        ...
        if backSponge:
            ...
            regions += [[old_div(((x1-backSponge)+x1),2.),
                         old_div(((y0+leftSponge)+(y1-rightSponge)),2.),
                         old_div((z0+z1),2.)]]
        ...
        if frontSponge:
            ...
            regions += [[old_div(((x0+frontSponge)+x0),2.),
                         old_div(((y0+leftSponge)+(y1-rightSponge)),2.),
                         old_div((z0+z1),2.)]]
        ...
        if leftSponge and backSponge:
            ...
            regions += [[old_div((x1+(x1-backSponge)),2.),
                         old_div((y0+(y0+leftSponge)),2.),
                         old_div((z0+z1),2.)]]
        ...
        if rightSponge and frontSponge:
            ...
            regions += [[old_div((x0+(x0+frontSponge)),2.),
                         old_div((y1+(y1-rightSponge)),2.),
                         old_div((z0+z1),2.)]]
        ...
        if frontSponge and leftSponge:
            ...
            v_i += 1
            r_i += 1
        for i in range(old_div(nb_corner,2)):
    ...
    def setGenerationZones(self, waves=None, windSpeed=(0. ,0., 0.),
        ...
        for key, value in self.abs_zones.items():
            if value is True:
                ...
                center = list(self.coords)
                if key == 'front':
                    center[0] += 0.5*self.dim[0]-old_div(sl['front'],2.)
                    orientation = [1., 0., 0.]
                elif key == 'back':
                    center[0] += -0.5*self.dim[0]+old_div(sl['back'],2.)
                    orientation = [-1., 0., 0.]
                elif key == 'left':
                    center[1] += -0.5*self.dim[1]+old_div(sl['left'],2.)
                    orientation = [0., 1., 0.]
                elif key == 'right':
                    center[1] += 0.5*self.dim[1]-old_div(sl['right'],2.)
                    orientation = [0., -1., 0.]
                self.zones[flag] = bc.RelaxationZone(shape=self,
                                                     zone_type='generation',
                                                     orientation=orientation,
...
class Tank2D(ShapeRANS):
    ...
    def __init__(self, domain, dim=(0., 0.), coords=None, from_0=True):
        ...
        self.spongeLayers = {'left': None,
                             'right': None}
        if coords is None:
            self.coords = old_div(self.dim,2.)
        else:
            self.coords = coords
            self.from_0 = False
        self.boundaryTags = {'bottom': 1,
    ...
    def setDimensions(self, dim):
        ...
        leftSponge = self.spongeLayers['left'] or 0.
        rightSponge = self.spongeLayers['right'] or 0.
        regions_y = y1-old_div(y1,100.)  # y coord of regions
        ...
        regions = [[old_div((x0+leftSponge+x1-rightSponge),2.), regions_y]]
        regionFlags = [1]
        ...
        if leftSponge:
            ...
            regions += [[old_div((x0+leftSponge),2.), regions_y]]
        ...
        if rightSponge:
            ...
            vertexFlags += [bt['bottom'], bt['top']]
            regions += [[old_div(((x1-rightSponge)+x1),2.), regions_y]]
    ...
    def setAbsorptionZones(self, left=False, right=False,
        ...
        if left is True:
            ...
            epsFact_solid = old_div(self.spongeLayers['left'],2.)
            orientation = [1., 0.]
            self.zones[flag] = bc.RelaxationZone(shape=self,
                                                 zone_type='absorption',
                                                 orientation=orientation,
        ...
        if right is True:
            ...
            flag = self.regionFlags[ind]
            center[0] += 0.5*self.dim[0]-old_div(self.spongeLayers['right'],2.)
            epsFact_solid = old_div(self.spongeLayers['right'],2.)
            orientation = [-1., 0.]
            self.zones[flag] = bc.RelaxationZone(shape=self,
                                                 zone_type='absorption',
                                                 orientation=orientation,
    ...
    def setGenerationZones(self, waves=None, windSpeed=(0., 0., 0.),
                           left=False, right=False,  dragAlpha=old_div(0.5,1.005e-6),
                           dragBeta=0., porosity=1.):
        waves = waves
        windSpeed = windSpeed
        if left or right:
        ...
        if left is True:
            ...
            ind = self.regionIndice['leftSponge']
            flag = self.regionFlags[ind]
            center[0] += -0.5*self.dim[0]+old_div(self.spongeLayers['left'],2.)
            epsFact_solid = old_div(self.spongeLayers['left'],2.)
            orientation = [1., 0.]
            self.zones[flag] = bc.RelaxationZone(shape=self,
                                                 zone_type='generation',
...
################################################################################################################################################################################################################################
        ...
        def static_case(self, sign, Fx, Fv, mass, m):
            ...
            if abs(Fx)<abs(Ftan):
                ...
                Fx = Fx+Ftan
                self.acceleration[0] = old_div(Fx,mass)
        ...
        def dynamic_case(self, sign, Fx, Fv, mass, m):
            ...
            for i in range(substeps):
                ...
                vel_check = self.acceleration[0]*dt_sub
                last_vel_check = self.last_acceleration[0]*dt_sub
                self.fromDynamic_toStatic = False 
                if (vel_check*last_vel_check) < 0.0:                                                      
                    self.velocity += np.zeros(3)     
                    self.h += np.zeros(3)
...
######### Friction module
        ...
        #---static case
                if self.last_velocity[0] == 0.0 or self.last_fromDynamic_toStatic ==True:
                    static_case(self, sign, Fx, Fv, mass, m=self.m_static)
        #---dynamic case
                else :
                    dynamic_case(self, sign, Fx, Fv, mass, m=self.m_dynamic)               
######### Floating module
            if (Fv*gv)<0:
        #---static case
                if self.last_velocity[0] == 0.0 or self.last_fromDynamic_toStatic ==True:
                    static_case(self, sign, Fx, Fv, mass, m=0.0)
        #---dynamic case
                else :
                    dynamic_case(self, sign, Fx, Fv, mass, m=0.0)
...
################################################################################################################################################################################################################################
        ...
        # acceleration from force:
            self.acceleration = old_div(self.F,self.Shape.mass)          
        # displacement
            for i in range(substeps):
                self.velocity += self.acceleration*dt_sub
                self.h += self.velocity*dt_sub
        ...
        if sum(self.M) != 0:
            ...
            ang_acc = old_div(self.M[:],self.inertia)
        else:
            self.inertia = None
...
def assembleDomain(domain):
    ...
    for shape in domain.shape_list:
        ...
        verticesFlags = shape.vertexFlags.copy()
        ...
        for i_s, vertex in enumerate(shape.vertices):
            if vertex.tolist() in domain.vertices:
                ...
                verticesFlags = np.delete(verticesFlags, i_s-del_v)
                i_s -= del_v
                del_v += 1
        ...
        domain.vertexFlags += (verticesFlags+start_flag).tolist()
