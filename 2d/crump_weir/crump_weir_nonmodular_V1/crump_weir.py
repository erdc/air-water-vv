...
obst_x_start = 7.0  # start x coordinate of the obstacle; caution to be in the domain's range
...
x_refine = (0.5 , 1.5 , 7.0) #end of zone 1, end of zone 2, end of zone 3 (zone 4 is up to the right wall)
...
downstream_water_level = 1.450
outflow_velocity = float(inflow_velocity)*float(waterLine_z)/float(downstream_water_level)
...
def outflowPressure(x,t):
   if x[1]>= downstream_water_level:
    ...
    return (L[1]-downstream_water_level)*rho_1*abs(g[1]) +  (downstream_water_level - x[1])*rho_0*abs(g[1])
    #p_L = L[1]*rho_1*g[1]
