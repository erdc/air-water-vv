...
def symmetric2D(box=(1.0,0.41),
                ...
                DX_coarse = 0.02):
    boundaryFlags = { 'bottom': 1, 'front':2, 'right':3, 'back': 4, 'left':5, 'top':6, 'obstacle':7}
    ...
    for i in range(0,top_nPoints+1):
        ...
        vertexFlags0.append(boundaryFlags['top'])
    ...
    for sN in range(len(vertices0)-1):
        ...
        if (vertices0[sN][0] == 0.0 and vertices0[sN+1][0] ==  0.0):
            ...
            segmentFlags0.append(boundaryFlags['top'])
        else:
    ...
    for i,v in enumerate(vertices_hold):
        ...
        if v[1]==0.0:
            ...
            if vertexFlags[i] == boundaryFlags['top']:
                vertexFlags.append(boundaryFlags['bottom'])
            else:
    ...
    for i,s in enumerate(segments_hold):
        ...
        if sN not in segments:
            segments.append(sN)
            if segmentFlags[i]==boundaryFlags['top']:
                segmentFlags.append(boundaryFlags['bottom'])
            else:
    ...
    vertexFlags.append(boundaryFlags['top'])
    ...
    segmentFlags.append(boundaryFlags['top'])
