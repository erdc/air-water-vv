...
ct = Context.Options([
    ...
    ("Refinement",          8, "Specify initial mesh size by giving number of cells in each direction"),
    ("spaceOrder",          1,"FE space for velocity"),
    ("parallel",            False,"Use parallel or not"),
    ("dt_fixed",            0.001,"fixed time step"),
    ("nDTout",              500,"output number"),
...
#===============================================================================
ball_center = np.array([[particle_diameter, L[1]-particle_diameter, particle_diameter],
                        ],'d')
...
if useHex:
    ...
    if structured:
        ...
        domain,boundaryTags, _, _ = my_domain.get_domain_one_box(L=L,
                                                           he=he)
...
dt_init = dt_fixed#0.5*dt_fixed#########HUGH EFFECT FOR EXTRAPOLATION
runCFL=0.3
nDTout = ct.nDTout
