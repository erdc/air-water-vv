...
opts=Context.Options([
    ...
    # Geometry
    #("tank_dim", (0.7, 7.5,), "Dimensions of the tank"),
    #("tank_sponge", (35., 0.), "Length of relaxation zones (front/back, left/right)"),
    ...
    ("cfl", 0.4,"Target cfl"),
    ("duration", 25., "Durarion of the simulation"),
    ...
    # obstacle dimensions (tweak to set up the geometry of the structure and overtopping tank)
...
wave_length = np.sqrt(9.81*waterLevel)*opts.Tp
tank_sponge = (wave_length, 0)

# --- WAVE input
Tend = 10.*opts.duration
x0 = np.array([0.,0.,0.])
Tp = opts.Tp
Hs = opts.Hs
...
obstacle = [
           [ [3*wave_length,0.],
            [3*wave_length+opts.structureCrestLevel*opts.structure_slope,opts.structureCrestLevel],
            [3*wave_length+opts.structureCrestLevel*opts.structure_slope,0.]
            ]
            ]
...
gauge_x = (3*wave_length+opts.structureCrestLevel*opts.structure_slope+opts.deposit_width/2)
