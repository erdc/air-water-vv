...
obst_portions = (0.001,0.25) #(width,distance from the bottom)
...
x_refine = (1.0 , 1.5 , 2.5 , 3.0) #end of zone 1, end of zone 2, end of zone 3 (zone 4 is up to the right wall)
...
if useHex:   
    ...
    boundaries=['empty','left','right','bottom','top','front','back','gate_v','gate_h']
    ...
    if structured:
        ...
        regions=[
            ...
            [x_refine[3]+0.00001, 0.00001] # Zone 5
            ]

        regionFlags=[1,2,3,4,5]
        trigArea = 0.5*he**2
        refinementArea =(refinementLevel[0]**2 , refinementLevel[1]**2)
        regionConstraints = [trigArea, trigArea*float(refinementArea[0]), trigArea*float(refinementArea[1]), trigArea*float(refinementArea[1]),trigArea*float(refinementArea[0])]
