  ...
  public:
    ...
    double *step(double *forces, double *torques, double dt)
    {
        ...
        for (int i = 0; i < num_particles; i++)
        {
            ...
            ChVector<> W = my_system.Get_bodylist()->at(i+1)->GetMass() * my_system.Get_G_acc();//since 1st body is ground
            my_system.Get_bodylist()->at(i+1)->Accumulate_force(myForce, pos, 0);//input force vector in the global coord
            my_system.Get_bodylist()->at(i+1)->Accumulate_torque(myTorque, 0);//input force vector in the global coord
        ...
        for (int i = 0; i < num_particles; i++)
        {
            ChVector<> pos = my_system.Get_bodylist()->at(i+1)->GetPos();
            pos_[i * 3 + 0] = pos.x();
            pos_[i * 3 + 1] = pos.y();
            pos_[i * 3 + 2] = pos.z();
            ChVector<> vel = my_system.Get_bodylist()->at(i+1)->GetPos_dt();
            vel_[i * 3 + 0] = vel.x();
            vel_[i * 3 + 1] = vel.y();
            vel_[i * 3 + 2] = vel.z();
            //return the coordinate of angular velocity vector in the absolute system
            ChVector<> angular_vel = my_system.Get_bodylist()->at(i+1)->  GetWvel_par();//return//since 1st body is ground
...
//    }
    ...
    void writeThisFrame()
    {
        ...
        {
            ChVector<> mypos = my_system.Get_bodylist()->at(j+1)->GetPos();//since 1st body is ground
            ChVector<> myvel = my_system.Get_bodylist()->at(j+1)->GetPos_dt();
            ChVector<> myangular_vel = my_system.Get_bodylist()->at(j+1)->  GetWvel_loc();
