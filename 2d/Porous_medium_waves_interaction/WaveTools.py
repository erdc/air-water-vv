...
"""
    def __init__(self,
        ...
        self.gAbs = sqrt(sum(g*g))#self.g[0]*self.g[0]+self.g[1]*self.g[1]+self.g[2]*self.g[2])
...
# Checking for
    ...
    def eta(self, x, t):
        if self.waveType is "Linear":
            ...
            phase = x[0]*self.kDir[0]+x[1]*self.kDir[1]+x[2]*self.kDir[2] - self.omega*t  + self.phi0
            HH = 0.
            ii =0.
    ...
    def u(self, x, t):
        if self.waveType is "Linear":
            ...
            for B in self.Bcoeff:
                ii+=1
                Bi=B*sqrt(self.gAbs/self.k)
                UH+=ii*Bi*cosh(ii*self.k*(Z+self.depth))*cos(ii*phase)/cosh(ii*self.k*self.depth)
                UV+=ii*Bi*sinh(ii*self.k*(Z+self.depth))*sin(ii*phase)/cosh(ii*self.k*self.depth)
                Ufenton = np.array([UH*self.waveDir[0]+UV*self.vDir[0],
