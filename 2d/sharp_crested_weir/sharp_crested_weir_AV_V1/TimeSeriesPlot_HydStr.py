...
if Datasource!=3:
 ...
 for ss in range (0,int(NumLocations)):
  ...
  filename_PV.append('PVGauges'+str(ss)+'.csv')  
  ...
  #read data
   if Datasource==1:
    ...
    y_p_PV[:,ss] /= float(rho_0)*9.81*float(inflowHeightMean) #based on linear theory
    y_phi_PV[:,ss]=-b2[:,9]+float(y_loc)
    y_phi_PV-=mean(y_phi_PV)
    if Variable==0:
      plot(time_P[:],y_phi_PV[:,ss],linetype[j],lw=1.0,label='PV') #,color = cs[j])
    elif Variable==1:
      plot(time_P[:],u_PV[:],linetype[j],lw=1.0,label='PV') #,color = cs[j])
    elif Variable==2:
      plot(time_P[:],v_PV[:],linetype[j],lw=1.0,label='PV') #,color = cs[j])
   ...
   for ts in time_P:
      y_theor[kk]=inflowHeightMean 
      u_theor[kk]=twpflowVelocity_u(loc,ts)
      v_theor[kk]=twpflowVelocity_u(loc,ts)
 ...
 # y_theor-=mean(y_theor) 
  ...
  #xlim(0.0,4.0)
   ...
   grid()
   title('x='+ str(x_loc[ss]) + '(m)',fontsize=16) 
   xticks(fontsize = 14) 
   yticks(fontsize = 14) 
   if Variable==0:
      ylim(-0.08,0.08)
  ...
  filename_ZC='FSEProbe_stats.txt'  
  ...
  xlim(int(GenerationZoneLength)-0.1,int(L[0])+0.1)
