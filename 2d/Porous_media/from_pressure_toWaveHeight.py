...
info.write('Probe No'+'\t'+'x'+'\t'+'Period'+'\t'+'Mean Height'+'\t' + 'Hrms'+'\t'+'H1/3'+'\t'+'H1/10'+'\t'+'Hmax'+'\t'+'Kr'+'\n')
#At FSEProbe_stats.txt it is printed: 1 Probe No / 2 Probe x/ 3 Probe y / 4 Avg Period / 5 Avg H / 6 Input H error / Input T error
for j in range(nprobes):
    info.write(str(j+1)+'\t'+str(probex[j])+'\t'+str(Tavg[j])+'\t'+str(Havg[j])+'\t' + str(Hrms[j])+'\t'+str(H3[j])+'\t'+str(H10[j])+'\t'+str(Hmax[j])+'\t'+str(Kr[j])+'\t'+'\n')
