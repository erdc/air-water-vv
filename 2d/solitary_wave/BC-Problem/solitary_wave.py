...
opts=Context.Options([
    ('lbc',False,"use low level boundary condtions"),
    # predefined test cases
    ("water_level", 1., "Height of free surface above seabed"),
    # tank
    ("tank_dim", (30, 3.0), "Dimensions of the tank"),
    ...
    ("tank_sponge", (0.0,0), "Length of relaxation zones zones (left, right)"),
    ...
    ("wave_height", 0.45, "Height of the waves"),
...
def setVertDir(g):
    return -np.array(g/(np.sqrt(g[0]**2 + g[1]**2 + g[2]**2)))
...
# ----- TANK ------ #
sloped_shore=[[[1.2,0.0],[7.8,1.2]]]
#tank = st.TankWithObstacles2D(domain, tank_dim, obstacles=sloped_shore)
...
fast = opts.fast
g = np.array([0.,-9.81,0.])
waveDir =  setDirVector(np.array(opts.wave_dir))
vDir = setVertDir(g)
gAbs = np.sqrt(g[0]*g[0]+g[1]*g[1]+g[2]*g[2])
trans = [0.,0.,0.]
depth = opts.water_level
Hh = H0/depth
c0 = np.sqrt((gAbs*depth)*(1+Hh))
mwl = opts.water_level
As  = np.sqrt((3./4.*H0/depth**3))
Fs = 6.0
Xs = depth*Fs/np.sqrt(Hh)
eta0 = 0.5

def get_inlet_ux_dirichlet(i):
    def ux_dirichlet(x, t):
        phi =  x[1] - (wave.eta(x,t)+depth)
        if phi >= smoothing:
            eta = 1.
        elif -smoothing < phi < smoothing:
            eta = smoothedHeaviside(smoothing, phi)
        elif phi <= -smoothing:
            eta = 0.
        vof = eta * air + (1 - eta) * water
        return (1.-vof)*wave.u(x,t)[i]
    return ux_dirichlet

def inlet_p_flux(x,t):
    phi =  x[1] - (wave.eta(x,t)+depth)
    if phi >= smoothing:
        eta = 1.
    elif -smoothing < phi < smoothing:
...
def inlet_vof_advective(x, t):
    ...
    if phi >= smoothing:
       ...
       eta = 0.
    vof = eta * air + (1 - eta) * water
    return -vof*wave.u(x,t)[0]

def waveHeightValue(x, t):
    return wave.eta(x,t)

print wave.eta([0.0,0.0,0.0],0.0)
###################################################
####Specifying Boundary condition##################
###################################################

tank.BC['x-'].u_dirichlet.uOfXT = get_inlet_ux_dirichlet(0)
tank.BC['x-'].v_dirichlet.uOfXT = get_inlet_ux_dirichlet(1)
tank.BC['x-'].w_dirichlet.uOfXT = get_inlet_ux_dirichlet(2)
tank.BC['x-'].vof_dirichlet.uOfXT = inlet_vof_dirichlet
tank.BC['x-'].p_advective.uOfXT = inlet_p_flux
...
epsFact_consrv_diffusion = 10. # default: 1.0. Test: 0.1
...
def twpflowPressure_init(x, t):
    p_L = 0.0
    phi_L = tank_dim[nd-1] - waveHeightValue(x, t) + depth 
    phi = x[nd-1] - waveHeightValue(x, t) + depth
