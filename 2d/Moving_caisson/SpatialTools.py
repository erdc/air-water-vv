...
class ShapeRANS(Shape):
    ...
    def getInertia(self, vec=(0., 0., 1.), pivot=None):
        ...
        distance = pivot-np.array(self.barycenter)            # Distance is (0.,0.,0.) when pivot=barycenter
    ...
    def setRecordValues(self, all_values=False, time=True, pos=False,
        ...
        if self.free_r[0] == 0. and self.free_r[1] == 0. and self.free_r[2] == 0.:
            self.record_bool = [time, pos, pos_x, pos_y, pos_z, F, Fx, Fy, Fz,
            ...
            self.record_bool = [time, pos, pos_x, pos_y, pos_z, rot, rot_x, rot_y,
                            ...
                            vel_x, vel_y, vel_z, acc_x, acc_y, acc_z, angvel_x, angvel_y, angvel_z, h1, h2, l3, l4, pivot_friction, ang_disp, Mp, rp]
        if all_values is True:
            self.record_bool = [True for value in self.record_bool]
        if self.free_r[0] == 0. and self.free_r[1] == 0. and self.free_r[2] == 0.:
            self.record_names = ['time', 'pos_x', 'pos_y', 'pos_z', 'Fx', 'Fy', 'Fz',
                                 'vel_x', 'vel_y', 'vel_z',
                                 'acc_x', 'acc_y', 'acc_z']
        else:
...
################################################################################################################################################################################################################################
        ...
        if self.ang != 0.:
            if self.friction == True:
                self.Shape.rotate(self.ang, self.angvel, self.pivot_friction)
            else:
