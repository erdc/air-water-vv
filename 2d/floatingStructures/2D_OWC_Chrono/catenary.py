...
class MooringLine:
    ...
    def setDirectionDistance(self):
        if self.nd == 3:
            ...
            #self.distance = np.abs(self.fairlead[0]-self.anchor[0])
            self.distance = np.abs(self.fairlead[0]-self.anchor[0])
            self.direction = np.array([(self.fairlead[0]-self.anchor[0])/self.distance, 0.])
    ...
    def setVariables(self):
        if self.broken is False:
            ...
            if fairlead2D[0] < anchor2D[0]:
                ...
                anchor2D = np.array([0., 0.])
                self.setDirectionDistance()
                fairlead2D = np.array([self.distance, self.fairlead[1]-self.anchor[1]])    
        ...
        if self.nd == 2:
            ...
            self.Ta = Ta*vec2D
            self.y = y_coords
            #self.s = lambda s: self.anchor+[s_coords(s)[0], s_coords(s)[1], 0.]
            #self.ds = lambda s: np.array([ds_coords(s)[0], ds_coords(s)[1], 0.])
            self.s = lambda s: self.anchor+[s_coords(s)[0]*self.direction[0], s_coords(s)[1], 0]
            self.ds = lambda s: np.array([ds_coords(s)[0]*self.direction[0], ds_coords(s)[1], 0])
        if self.nd == 3:
            Tf3D = np.zeros(3)                
            Ta3D = np.zeros(3)
