...
vals = np.loadtxt("vegZoneVelocity.csv",
                  skiprows = 1,
                  delimiter=',')

f = open("vegZoneVelocity.csv", 'r')
first = f.readline()
f.close()
g = first.split(',')
zListU=[]
...
interpU = interp.RectBivariateSpline(time,zU,Uarray, kx=1,ky=3)
interpW = interp.RectBivariateSpline(time,zW, Warray, kx=1, ky=3)
#plt.contourf(tt,zzU,Uarray)
#plt.imshow(Uarray)
#plt.show()
vals = np.loadtxt("column_gauge.csv",
                  skiprows=1,
                  delimiter=',')
