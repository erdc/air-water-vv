...
def fully_lifted_rigid(d, h, L, tol=1e-6, maxit=1000):
    g = lambda a: 2.*a*np.sinh(d/(2.*a))-np.sqrt((Ls0)**2.-h**2.)
    dg = lambda a: 2.*np.sinh(d/(2.*a))-d*np.cosh(d/(2.*a))/a
    a0 = bisection(f=g, int1=1., int2=1e15, tol=tol, maxit=maxit)
    a1 = newton_raphson(f=g, df=dg, x0=a, tol=tol, maxit=maxit)      
    if np.isnan(a1) or a < 0:
        a = a0
    else:
        a = a1
    return a
...
class MooringLine:
    ...
    def setVariables(self):
        ...
        if state == 'part':
            ...
            xs = [0]+[a*np.arcsinh((np.sum(Ls[:i+1])+scorr)/a)-k for i in range(len(Ls))]
            y_coords = lambda x: a*(np.cosh((x+k)/a))+m  # y coordinates of points along the
            s_coords = lambda s: [a*np.arcsinh(s/a)-k, a*np.cosh((a*np.arcsinh(s/a)-k)/a)+m]
        elif state == 'straight':
