...
fig4.legend((line1,line2,line3), ('Total','Incindent','Reflected'),'right', bbox_to_anchor=(0.85,0.8)) 
...
for j in range(nprobes):
    info.write(str(j+1)+'\t\t'+str(probex[j])+'\t\t'+str(Tavg[j])+'\t'+str(Havg[j])+'\t' + str(Hi[j])+'\t'+str(H3[j])+'\t'+str(H10[j])+'\t'+str(Hmax[j])+'\t'+'\n')
