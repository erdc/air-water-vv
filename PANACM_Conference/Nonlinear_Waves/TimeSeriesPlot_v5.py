 ...
 for ss in range (0,int(NumLocations),10):
  ...
  #read data
   if Datasource==1:
    ...
    time_P = b3[:,0]
    if Variable==0:
     plot(time_P[:],y_VOF[:,ss],linetype[j],lw=1.0,label='numerical') #,color = cs[j])
 ...
 # y_theor-=mean(y_theor) 
   ...
   if j==0 :
    if Variable==0:
       ...
       plot(time_P[:],u_theor[:],'r--',lw=1.0,label='Analytical')#,color = cs[j])
    elif  Variable==2:
       plot(time_P[:],v_theor[:],'r--',lw=1.0,label='Analytical')#,color = cs[j])
